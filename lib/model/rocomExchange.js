...
function parseExchangeArgs(text) {
...
function normalizePoster(poster) {
    const userInfo = poster.user_info && typeof poster.user_info === 'object' && !Array.isArray(poster.user_info) ? poster.user_info : {};
...
async function getRocomExchangeHall(event) {
    ...
    const payload = await requestWeGame('/api/v1/games/rocom/exchange/posters', {
        method: 'GET',
        headers: {
...
function buildRocomExchangeText(payload) {
...
export { buildRocomExchangeText, getRocomExchangeHall };
