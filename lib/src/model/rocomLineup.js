...
function parseLineupListArgs(rawText) {
...
function parseLineupId(rawText) {
...
function normalizeLineupCard(lineup) {
    const lineupPets = lineup.lineup && typeof lineup.lineup === 'object' && !Array.isArray(lineup.lineup)
        ? lineup.lineup.pets
        : [];
    return {
...
async function getRocomLineupList(event, rawArgs = '') {
    ...
    const payload = await requestWeGame('/api/v1/games/rocom/lineup/list', {
        method: 'GET',
        headers: {
...
function buildRocomLineupListText(payload) {
...
async function getRocomLineupDetail(event, rawArgs = '') {
...
function buildRocomLineupDetailText(payload) {
    return [
        payload.name,
        `阵容码：${payload.id}`,
...
export { buildRocomLineupDetailText, buildRocomLineupListText, getRocomLineupDetail, getRocomLineupList };
