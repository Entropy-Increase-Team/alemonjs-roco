...
import { getWeGameUserContext, resolveActiveWeGameCredential, requestWeGame } from './wegameAccount.js';
...
async function getPageSize() {
    const rocom = (await readRocomConfig()).rocom;
...
function getLocalPetTypes(pet) {
    const values = [pet.main_type, pet.sub_type]
        ...
        return normalizeText(localized.zh);
    })
        .filter(Boolean);
    return values.length > 0 ? values.join(' / ') : '';
}
function resolveLocalPetByIdOrName(value) {
    const text = normalizeText(value);
    if (!text) {
        return null;
    }
    const pets = getPetsData();
    return (pets.find(item => String(item.id) === text) ??
        pets.find(item => getLocalPetName(item) === text) ??
        pets.find(item => normalizeText(item.name).toLowerCase() === text.toLowerCase()) ??
        null);
}
function resolveZone(loginType) {
...
async function parsePetListArgs(text) {
...
function extractPetList(payload, depth = 0) {
    ...
    for (const value of Object.values(payload)) {
        if (value && typeof value === 'object' && !Array.isArray(value)) {
            const rows = extractPetList(value, depth + 1);
            if (rows.length > 0) {
                return rows;
            }
        }
...
function extractTypesFromPayload(pet) {
...
function getPetRarityLabel(subset) {
    return subset === 0 ? '常规' : getPetSubsetLabel(subset);
}
function getRoleProfile(payload) {
    return payload.role && typeof payload.role === 'object' && !Array.isArray(payload.role) ? payload.role : {};
}
function buildPetListItem(pet, index, pageNo, petSubset, pageSize) {
    ...
    const imageUrl = normalizeUrl(pet.pet_img_url ?? pet.image_url ?? pet.img_url ?? pet.pet_image ?? pet.pet_img ?? pet.avatar ?? pet.icon ?? pet.icon_url ?? pet.cover ?? pet.pic) || '';
    return {
        name,
...
async function getRocomPetList(event) {
    ...
    const [petData, roleProfile] = await Promise.all([
        requestWeGame('/api/v1/games/rocom/battle/pets', {
            method: 'GET',
            headers: {
    ...
    return {
        ...
        totalPages: Math.max(1, toNumber(petData.total_pages ?? petData.totalPages ?? petData.page_count ?? petData.pageCount, Math.ceil(toNumber(petData.total ?? petData.total_count ?? pets.length, pets.length) / pageSize) || 1)),
        pageSize,
        pets: pets.map((item, index) => buildPetListItem(item, index, args.pageNo, args.petSubset, pageSize))
    };
}
function buildRocomPetListText(payload) {
    const lines = [
        '我的精灵',
...
export { buildRocomPetListText, getRocomPetList };
