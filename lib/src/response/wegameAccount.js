...
var wegameAccount = async () => {
    ...
    try {
        if (routeKey === 'wgqq登陆') {
            ...
            void message.send({
                format: buildTextImageFormat(['请使用另外一台设备的 QQ 扫描下方链接中的二维码完成 WeGame 登录。', `二维码地址：${qrImage}`, '登录成功后会自动同步到账号绑定列表。'].join('\n'), qrImage)
            });
        ...
        if (routeKey === 'wgwx登陆') {
            ...
            void message.send({
                format: buildTextImageFormat(['请使用另外一台设备的微信扫描下方链接中的二维码完成 WeGame 登录。', `二维码地址：${qrImage}`, '登录成功后会自动同步到账号绑定列表。'].join('\n'), qrImage)
            });
