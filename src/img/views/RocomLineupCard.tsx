...
function renderPetChip(pet: LineupPet, index: number, mode: 'list' | 'detail') {
    ...
    >
      ...
      >
        {pet.imageUrl ? (
          ...
          <div style={{ fontSize: mode === 'detail' ? '30px' : '24px', fontWeight: 900, color: 'rgba(0,0,0,0.2)' }}>{pet.name.slice(0, 1)}</div>
        )}
      </div>
      <div
        style={{
...
export default function RocomLineupCard({ data }: Props) {
  ...
  return (
    <HTML style={{ width: `${width}px`, background: 'linear-gradient(180deg, #181919 0%, #101111 100%)' }}>
      <div
        style={{
          width: `${width}px`,
          ...
          padding: '28px',
          background:
            'radial-gradient(circle at top left, rgba(255,173,84,0.18), transparent 22%), radial-gradient(circle at top right, rgba(255,255,255,0.1), transparent 20%), linear-gradient(180deg, #2b211b 0%, #161311 100%)',
          color: '#f5efdf'
        }}
      >
        <div
        ...
        >
          <div>
            <div style={{ fontSize: '36px', fontWeight: 900, letterSpacing: '2px' }}>{title}</div>
            <div style={{ marginTop: '8px', fontSize: '16px', color: '#d6c2a4' }}>{data.category || (data.mode === 'detail' ? '阵容单卡' : '热门推荐')}</div>
          </div>
          <div style={{ textAlign: 'right', fontSize: '15px', color: 'rgba(245,239,223,0.74)' }}>
            <div>
              第 {data.pageNo} / {data.totalPages} 页
            </div>
            <div style={{ marginTop: '6px' }}>{data.mode === 'detail' ? '查看阵容 <阵容码>' : '阵容 <分类> <页码>'}</div>
          </div>
        </div>
          ...
          {data.lineups.map((lineup, index) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: data.mode === 'detail' ? '30px' : '26px', fontWeight: 900, lineHeight: 1.1 }}>{lineup.name}</div>
