...
import { getRocomConfigSource, getWeGameCoreConfigSource, readRocomConfig, readWeGameCoreConfig } from '@src/model/wegameResource';
...
function getValueByPath(payload: Record<string, unknown>, pathText: string): unknown {
...
async function getCoreWeGameConfig(): Promise<ConfigSection> {
...
async function getRocomConfig(): Promise<ConfigSection> {
...
async function getRocomMerchantRuntimeSummary(): Promise<ConfigSection> {
  const payload = await readRuntimeStore<Record<string, unknown>>(runtimeSubscriptionStoreKey, runtimeSubscriptionStoreFileName, {});
...
export function getWeGameConfigRegistry(): ConfigSection[] {
  return [
    {
      code: 'wegame-core',
      title: 'WeGame 主插件',
...
export async function buildWeGameConfigSummary(code?: string): Promise<string> {
...
export function buildWeGameConfigRegistryText(): string {
